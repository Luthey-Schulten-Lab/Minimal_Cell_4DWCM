...
import diffusion_functions as Diff
...
def general_reaction_rates(sim):
    ...
    deg_bind_rate = 11*avgdr*Ecoli_V/60/7800 #2400 #4800 #7800 #1/M/s
...
#     sim.region('p_start_c').addReaction([mRNA, ribo_part], [mRNA_ribo], sim.rc.ribo_bind)
#     sim.region('p_mid_c').addReaction([mRNA, ribo_part], [mRNA_ribo], sim.rc.ribo_bind)
#     sim.region('p_end_c').addReaction([mRNA, ribo_part], [mRNA_ribo], sim.rc.ribo_bind)

#     sim.region('p_starts').addReaction([mRNA, ribo_part], [mRNA_ribo], sim.rc.ribo_bind)
#     sim.region('p_mids').addReaction([mRNA, ribo_part], [mRNA_ribo], sim.rc.ribo_bind)
#     sim.region('p_ends').addReaction([mRNA, ribo_part], [mRNA_ribo], sim.rc.ribo_bind)
...
#         mRNA_check = sim.species('RB_'+locusNum+'_cp')
#         mRNA_poly = sim.species('RB_'+locusNum+'_p')
#         mRNA_poly_f = sim.species('RB_'+locusNum+'_pe')

#         poly_120 = sim.rateConst(locusNum + '_poly_120', GIP.TranslationRate(aasequence[:39]), 1)
#         poly_end = sim.rateConst(locusNum + '_poly_end', GIP.TranslationRate(aasequence[40:]), 1)
        
#         sim.region('p_start_c').addReaction([mRNA_ribo], [mRNA_check], poly_120)
#         sim.region('p_start_c').addReaction([mRNA_poly], [ribo_part, ptnCost], poly_end)
#         sim.region('p_start_c').addReaction([mRNA_poly_f], [ribo_part, mRNA_read, ptnCost], poly_end)
        
#         sim.region('p_mid_c').addReaction([mRNA_ribo], [mRNA_check], poly_120)
#         sim.region('p_mid_c').addReaction([mRNA_poly], [ribo_part, ptnCost], poly_end)
#         sim.region('p_mid_c').addReaction([mRNA_poly_f], [ribo_part, mRNA_read, ptnCost], poly_end)
        
#         sim.region('p_end_c').addReaction([mRNA_ribo], [ribo_part, mRNA_read, ptnCost], translation_rate)
#         sim.region('p_end_c').addReaction([mRNA_check], [ribo_part, mRNA_read, ptnCost], poly_end)
#         sim.region('p_end_c').addReaction([mRNA_poly], [ribo_part, ptnCost], poly_end)
#         sim.region('p_end_c').addReaction([mRNA_poly_f], [ribo_part, mRNA_read, ptnCost], poly_end)
        
#         sim.region('p_starts').addReaction([mRNA_ribo], [mRNA_check], poly_120)
#         sim.region('p_starts').addReaction([mRNA_poly], [ribo_part, ptnCost], poly_end)
#         sim.region('p_starts').addReaction([mRNA_poly_f], [ribo_part, mRNA_read, ptnCost], poly_end)
        
#         sim.region('p_mids').addReaction([mRNA_ribo], [mRNA_check], poly_120)
#         sim.region('p_mids').addReaction([mRNA_poly], [ribo_part, ptnCost], poly_end)
#         sim.region('p_mids').addReaction([mRNA_poly_f], [ribo_part, mRNA_read, ptnCost], poly_end)
        
#         sim.region('p_ends').addReaction([mRNA_ribo], [ribo_part, mRNA_read, ptnCost], translation_rate)
#         sim.region('p_ends').addReaction([mRNA_check], [ribo_part, mRNA_read, ptnCost], poly_end)
#         sim.region('p_ends').addReaction([mRNA_poly], [ribo_part, ptnCost], poly_end)
#         sim.region('p_ends').addReaction([mRNA_poly_f], [ribo_part, mRNA_read, ptnCost], poly_end)
        
#         sim.region('ribosomes').addReaction([mRNA_check], [mRNA_read, ribo_part, ptnCost], poly_end)
#         sim.region('ribosomes').addReaction([mRNA_poly], [ribo_part, ptnCost], poly_end)
#         sim.region('ribosomes').addReaction([mRNA_poly_f], [mRNA_read, ribo_part, ptnCost], poly_end)
        
#         sim.region('ribo_centers').addReaction([mRNA_check], [mRNA_read, ribo_part, ptnCost], poly_end)
#         sim.region('ribo_centers').addReaction([mRNA_poly], [ribo_part, ptnCost], poly_end)
#         sim.region('ribo_centers').addReaction([mRNA_poly_f], [mRNA_read, ribo_part, ptnCost], poly_end)
        
#     else:
        
#         sim.region('p_start_c').addReaction([mRNA_ribo], [mRNA_read, ribo_part,  ptnCost], translation_rate)
#         sim.region('p_mid_c').addReaction([mRNA_ribo], [mRNA_read, ribo_part, ptnCost], translation_rate)
#         sim.region('p_end_c').addReaction([mRNA_ribo], [mRNA_read, ribo_part, ptnCost], translation_rate)

#         sim.region('p_starts').addReaction([mRNA_ribo], [mRNA_read, ribo_part, ptnCost], translation_rate)
#         sim.region('p_mids').addReaction([mRNA_ribo], [mRNA_read, ribo_part, ptnCost], translation_rate)
#         sim.region('p_ends').addReaction([mRNA_ribo], [mRNA_read, ribo_part, ptnCost], translation_rate)
...
#     RNAP_gene_trsc = sim.species('RP_' + locusNum + '_t')
...
#     sim.region('DNA').addReaction([RNAP_gene], [gene, RNApol], sim.rc.RNAP_off)
...
#     sim_properties['deg_states']['D_' + locusNum] = 0
...
#         RNAP_finished = 'RP_' + locusNum + '_f_' + str(i)

#         sim_properties['counts'][RNAP_finished] = 0

#         RNAP_gene_ready = 'RP_' + locusNum + '_ti_' + str(i)

#         sim_properties['counts'][RNAP_gene_ready] = 0
...
#             sim.region('DNA').addReaction([RNAP_gene], [RNAP_gene_trsc_m1, RNApol], sim.rc.RNAP_off)
...
def addRibosomeBiogenesis(sim, sim_properties):
    ...
    maxImts = 19 #145 # cfg.maxSpecies - (len(cfg.species) - 2) # Don't count 30S or 16S
